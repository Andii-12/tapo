...
export function PaymentModal({
  ...
  token: string;
  priceText: string;
  /** Original price shown struck-through during free sale. */
  listPrice?: number;
  isFree?: boolean;
  onPaid: () => Promise<void>;
}) {
  const [status, setStatus] = useState(
    isFree ? "Үнэгүй нээхэд бэлэн" : "Төлбөр хүлээгдэж байна"
  );
  ...
  async function ensureOrder() {
    if (session?.paymentRef) return session;
    if (!orderId) {
      setStatus("Захиалгын дугаар олдсонгүй");
      return null;
    }
    setLoading(true);
    setStatus(isFree ? "Үнэгүй нээж байна…" : "Төлбөр бэлтгэж байна…");
    try {
      const url =
        product === "natal"
      ...
      setSession(next);
      if (json.data.status === "paid" || json.data.amount === 0) {
        setStatus("Амжилттай нээгдлээ");
        return next;
      }
  ...
  async function unlockFree() {
    try {
      ...
      if (!created) return;
      if (created.status === "paid" || created.amount === 0) {
        setLoading(true);
        try {
          await onPaid();
        } finally {
      ...
      setStatus(e instanceof Error ? e.message : "Нээж чадсангүй");
    }
  }
  ...
  async function checkStatus() {
    ...
    try {
      ...
      if (
        ...
        json.data.paymentStatus === "not_required"
      ) {
  ...
  async function mockPay() {
    ...
    try {
      const created = await ensureOrder();
      if (!created?.paymentRef) return;
      const res = await fetch("/api/payments/mock-complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ paymentRef: created.paymentRef }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Алдаа");
      setStatus("Төлбөр амжилттай");
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isFree ? "Үнэгүй нээх" : "Төлбөр төлөх"}
    >
      <div className="space-y-4 text-sm">
        ...
        {isFree && listPrice != null ? (
          <SalePrice listPrice={listPrice} size="lg" />
        ) : (
        ...
        {isFree ? (
          <>
            <p className="text-ink-muted">
              Хямдралтай — одоо үнэгүй. Доорх товчоор бүрэн тайлбарыг нээнэ үү.
            </p>
            <p aria-live="polite">{status}</p>
            <div className="flex flex-wrap gap-3">
              <Button onClick={unlockFree} loading={loading} fullWidth>
                Үнэгүй нээх
              </Button>
